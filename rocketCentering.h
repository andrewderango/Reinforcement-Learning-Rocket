#ifndef rocketCentering_h
#define rocketCentering_h
...
/******************************************************************************/
class rocketCentering {
    protected:
    // parameters for simulation
        const double MASSROCKET = 2.0;
    ...
    public:
        ...
        double getRocketXPos() { return state[X]; }
        double getRocketXVel() { return state[V]; }
        void setDraw(bool d) { draw_track = d; }

        /************************************************************************/
        void draw(const int& action) {
            clearScreen();
            bool solved = (abs(state[X]) <= NEAR_ORIGIN && abs(state[V]) <= NEAR_ORIGIN);
